...
def timer(fn):
    def inner(*args, **kwargs):
        ...
        logging.info('{0} took {1:.2f}s to execute'.format(fn.__name__, execution_time))
...
def clean_dataset(dataset_name):
    input_dataset_name = dataset_name.lower() + "_raw"
    output_dataset_name = dataset_name.lower() + "_clean"
    response = build_response(dataset_name)
    df = None

    if dataset_name == "dataset_1":
        df = df_1_cleaning(get_ds(input_dataset_name))
    elif dataset_name == "dataset_3a":
        df = df_3a_cleaning(get_ds(input_dataset_name))
    elif dataset_name == "dataset_3b":
        df = df_3b_cleaning(get_ds(input_dataset_name))
    elif dataset_name == "dataset_4":
        df = df_4_cleaning(get_ds(input_dataset_name))
    elif dataset_name == "dataset_5":
        df = df_5_cleaning(get_ds(input_dataset_name))
...
def get_ds(table):
    ...
    if(check_table(table)):
        ...
        if(re.match(regex_clean, table)):
            ...
            raw_ds = get_ds(raw_ds_name)

            if table == "dataset_1_clean":
                df = df_1_cleaning(raw_ds)
            elif table == "dataset_3a_clean":
                df = df_3a_cleaning(raw_ds)
            elif table == "dataset_3b_clean":
                df = df_3b_cleaning(raw_ds)
            elif table == "dataset_4_clean":
...
def df_5_cleaning(df):
    ...
    df = df.groupby(["month","year","origin_country"]).agg({'cases':'sum','deaths':'sum'}).reset_index()
...
def merge_df(dfs):
    ...
    if df_count == 0:
        logging.error("No df given, you need to provide 2 at least.")
    elif df_count == 1:
        logging.error("Only one df found, you need to provide 2 at least.")
    elif df_count == 2:
        ...
        df = None
        logging.error("More than 5 dataframes are not supported.")
